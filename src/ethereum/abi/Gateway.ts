...
export const events = {
    AgentCreated: event("0x7c96960a1ebd8cc753b10836ea25bd7c9c4f8cd43590db1e8b3648cb0ec4cc89", {"agentID": p.bytes32, "agent": p.address}),
    AgentFundsWithdrawn: event("0xf953871855f78d5ccdd6268f2d9d69fc67f26542a35d2bba1c615521aed57054", {"agentID": indexed(p.bytes32), "recipient": indexed(p.address), "amount": p.uint256}),
    ChannelCreated: event("0xe7e6b36c9bc4c7817d3879c45d6ce1edd3c61b1966c488f1817697bb0b704525", {"channelID": indexed(p.bytes32)}),
    ChannelUpdated: event("0x66e174b5e03ba247add8660a34e70bdd484239fe794c2567772e8e93a5c1696b", {"channelID": indexed(p.bytes32)}),
    InboundMessageDispatched: event("0x617fdb0cb78f01551a192a3673208ec5eb09f20a90acf673c63a0dcb11745a7a", {"channelID": indexed(p.bytes32), "nonce": p.uint64, "messageID": indexed(p.bytes32), "success": p.bool}),
    OperatingModeChanged: event("0x4016a1377b8961c4aa6f3a2d3de830a685ddbfe0f228ffc0208eb96304c4cf1a", {"mode": p.uint8}),
    OutboundMessageAccepted: event("0x7153f9357c8ea496bba60bf82e67143e27b64462b49041f8e689e1b05728f84f", {"channelID": indexed(p.bytes32), "nonce": p.uint64, "messageID": indexed(p.bytes32), "payload": p.bytes}),
    PricingParametersChanged: event("0x5e3c25378b5946068b94aa2ea10c4c1e215cc975f994322b159ddc9237a973d4", {}),
    TokenRegistrationSent: event("0xf78bb28d4b1d7da699e5c0bc2be29c2b04b5aab6aacf6298fe5304f9db9c6d7e", {"token": p.address}),
    TokenSent: event("0x24c5d2de620c6e25186ae16f6919eba93b6e2c1a33857cc419d9f3a00d6967e9", {"token": indexed(p.address), "sender": indexed(p.address), "destinationChain": indexed(p.uint32), "destinationAddress": p.struct({"kind": p.uint8, "data": p.bytes}), "amount": p.uint128}),
    TokenTransferFeesChanged: event("0x4793c0cb5bef4b1fdbbfbcf17e06991844eb881088b012442af17a12ff38d5cd", {}),
    Upgraded: event("0xbc7cd75a20ee27fd9adebab32041f755214dbc6bffa90cc0225b39da2e5c2d3b", {"implementation": indexed(p.address)}),
}

export const functions = {
    AGENT_EXECUTOR: viewFun("0x423e69b6", {}, p.address),
    BEEFY_CLIENT: viewFun("0x90ffc4f9", {}, p.address),
    agentExecute: fun("0x35ede969", {"data": p.bytes}, ),
    agentOf: viewFun("0x5e6dae26", {"agentID": p.bytes32}, p.address),
    channelNoncesOf: viewFun("0x2a6c3229", {"channelID": p.bytes32}, {"_0": p.uint64, "_1": p.uint64}),
    channelOperatingModeOf: viewFun("0x0705f465", {"channelID": p.bytes32}, p.uint8),
    createAgent: fun("0xc3b8ec8e", {"data": p.bytes}, ),
    createChannel: fun("0x17abcf60", {"data": p.bytes}, ),
    dropRescueAbility: fun("0xcd5a76de", {}, ),
    implementation: viewFun("0x5c60da1b", {}, p.address),
    initialize: fun("0x439fab91", {"data": p.bytes}, ),
    isTokenRegistered: viewFun("0x26aa101f", {"token": p.address}, p.bool),
    operatingMode: viewFun("0x38004f69", {}, p.uint8),
    pricingParameters: viewFun("0x0b617646", {}, {"_0": p.uint256, "_1": p.uint128}),
    quoteRegisterTokenFee: viewFun("0x805ce31d", {}, p.uint256),
    quoteSendTokenFee: viewFun("0x928bc49d", {"token": p.address, "destinationChain": p.uint32, "destinationFee": p.uint128}, p.uint256),
    registerToken: fun("0x09824a80", {"token": p.address}, ),
    rescue: fun("0x3c9c3b45", {"impl": p.address, "implCodeHash": p.bytes32, "initializerParams": p.bytes}, ),
    rescueOperator: viewFun("0xeac09209", {}, p.address),
    sendToken: fun("0x52054834", {"token": p.address, "destinationChain": p.uint32, "destinationAddress": p.struct({"kind": p.uint8, "data": p.bytes}), "destinationFee": p.uint128, "amount": p.uint128}, ),
    setOperatingMode: fun("0x8257f3d5", {"data": p.bytes}, ),
    setPricingParameters: fun("0x0c86ea46", {"data": p.bytes}, ),
    setTokenTransferFees: fun("0x5b2e9c4c", {"data": p.bytes}, ),
    submitV1: fun("0xdf4ed829", {"message": p.struct({"channelID": p.bytes32, "nonce": p.uint64, "command": p.uint8, "params": p.bytes, "maxDispatchGas": p.uint64, "maxFeePerGas": p.uint256, "reward": p.uint256, "id": p.bytes32}), "leafProof": p.array(p.bytes32), "headerProof": p.struct({"header": p.struct({"parentHash": p.bytes32, "number": p.uint256, "stateRoot": p.bytes32, "extrinsicsRoot": p.bytes32, "digestItems": p.array(p.struct({"kind": p.uint256, "consensusEngineID": p.bytes4, "data": p.bytes}))}), "headProof": p.struct({"pos": p.uint256, "width": p.uint256, "proof": p.array(p.bytes32)}), "leafPartial": p.struct({"version": p.uint8, "parentNumber": p.uint32, "parentHash": p.bytes32, "nextAuthoritySetID": p.uint64, "nextAuthoritySetLen": p.uint32, "nextAuthoritySetRoot": p.bytes32}), "leafProof": p.array(p.bytes32), "leafProofOrder": p.uint256})}, ),
    transferNativeFromAgent: fun("0x9a870c8b", {"data": p.bytes}, ),
    updateChannel: fun("0xafce33c4", {"data": p.bytes}, ),
    upgrade: fun("0x25394645", {"data": p.bytes}, ),
}
